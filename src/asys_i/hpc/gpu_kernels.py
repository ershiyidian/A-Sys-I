...
try:
    ...
    log.warning("GPU Kernels: transformer_engine and nvcomp not imported (simulation mode).")
except ImportError:
    TRANSFORMER_ENGINE_AVAILABLE = False
    NVCOMP_AVAILABLE = False
    log.warning("GPU Kernels: transformer_engine or nvcomp not found. Advanced modes disabled.")
...
def preprocess_tensor_on_gpu(
    ...
    if not CUDA_AVAILABLE:
         ...
         monitor.log_metric("gpu_kernel_error_count", 1, tags={**tags, "reason": "no_cuda"})
         return tensor, meta
    ...
    try:
        ...
        if mode == "FP8":
            if TRANSFORMER_ENGINE_AVAILABLE:
                 ...
                 monitor.log_metric("gpu_kernel_error_count", 1, tags={**tags, "reason": "missing_lib"})
             ...
             if NVCOMP_AVAILABLE and TRANSFORMER_ENGINE_AVAILABLE:
                  ...
                  # meta['compression_ratio'] = original_bytes / compressed_bytes
                   ...
                   log.error("FP8_LZ4 mode requested but Transformer Engine or NVCOMP not available.")
                   monitor.log_metric("gpu_kernel_error_count", 1, tags={**tags, "reason": "missing_lib"})

        else:
             log.warning(f"Unknown GPU kernel mode: {mode}. Returning original tensor.")
             monitor.log_metric("gpu_kernel_error_count", 1, tags={**tags, "reason": "unknown_mode"})
        ...
        # Note: Actual latency logging should happen after stream sync in hooker
         ...
         monitor.log_metric("gpu_kernel_error_count", 1, tags={**tags, "reason": "exception"})
         return tensor, {"error": str(e), "mode": mode} # Return original tensor on error
