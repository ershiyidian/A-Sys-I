...
def bind_current_process(
    config: MasterConfig, component_id: ComponentID, monitor: BaseMonitor
) -> None:
    """
    ...
    if not _IS_LINUX:
        log.warning(
            f"CPU/NUMA binding requested on non-Linux system ({platform.system()}) for {component_id}. Skipping."
        )
        monitor.log_metric(
            "resource_binding_skip_count", 1, tags={**tags, "reason": "not_linux"}
        )
        return
    ...
    try:
        ...
        if cores:
            ...
            if len(valid_cores) != len(cores):
                log.warning(
                    f"Invalid core IDs specified for {component_id}: {cores}. Using valid: {valid_cores}"
                )
                monitor.log_metric(
                    "resource_binding_error_count",
                    1,
                    tags={**tags, "reason": "invalid_cores"},
                )
            if valid_cores:
                p.cpu_affinity(valid_cores)
                log.info(
                    f"Bound process {pid} ({component_id}) to CPU cores: {p.cpu_affinity()}"
                )
                monitor.log_metric(
                    "resource_binding_success_count", 1, tags={**tags, "type": "cpu"}
                )
            else:
        ...
        if numa_node is not None:
            ...
            log.warning(
                f"NUMA binding ({numa_node}) for {pid} ({component_id}) requested. "
                "psutil binding is CPU-only. Use `numactl` to launch for memory/node binding."
            )
            monitor.log_metric(
                "resource_binding_skip_count",
                1,
                tags={**tags, "reason": "numa_unsupported"},
            )
            # Example:
        ...
        log.error(f"Error binding process {pid} ({component_id})")
        monitor.log_metric(
            "resource_binding_error_count", 1, tags={**tags, "reason": "ProcessError"}
        )
    except Exception:
        log.exception(f"Unexpected error during resource binding for {component_id}:")
        monitor.log_metric(
            "resource_binding_error_count", 1, tags={**tags, "reason": "exception"}
        )
