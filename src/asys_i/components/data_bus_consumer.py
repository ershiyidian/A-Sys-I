...
class PythonQueueBus(BaseDataBus):
    ...
    def __init__(self, config: DataBusConfig, monitor: BaseMonitor):
        ...
        if config.num_shards > 1:
            log.warning(
                f"PythonQueueBus does not support sharding. Ignoring num_shards={config.num_shards}."
            )
        ...
        self._drop_count = 0
        log.info(
            f"PythonQueueBus initialized with maxsize={config.buffer_size_per_shard}"
        )
    ...
    def push(self, packet: ActivationPacket, timeout: Optional[float] = None) -> bool:
        ...
        try:
            ...
            self.monitor.log_metric(
                "data_bus_queue_depth", self.queue.qsize(), tags=tags
            )
            self._push_count += 1
            return True
        except queue.Full:
            ...
            self.monitor.log_metric(
                "data_bus_error_count", 1, tags={**tags, "type": "push"}
            )
            return False
    ...
    def pull_batch(
        ...
        try:
            ...
            self.monitor.log_metric(
                "data_bus_error_count", 1, tags={**tags, "type": "pull"}
            )
