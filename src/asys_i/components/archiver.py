...
from asys_i.common.types import (
     ActivationPacket, ComponentID, LayerIndex, RunProfile,
 )
...
def archiver_worker_process_target(
     ...
     worker = ArchiverWorker(worker_id, config, data_bus, monitor, stop_event, all_layer_indices)
     try:
         worker.run()
     except Exception as e:
         log.exception(f"ArchiverWorker {worker_id} crashed:")
         monitor.log_metric("worker_crash_count", 1, tags={"component": worker_id, "type": "archiver"})
...
class ArchiverWorker:
     ...
     def __init__(self,
        ...
        if not PYARROW_AIO_AVAILABLE:
             log.error(f"Worker {worker_id}: PyArrow/aiofiles unavailable. Archiving disabled.")
     ...
     async def _write_buffer_async(self, layer_idx: LayerIndex, packets: List[ActivationPacket]):
          if not packets or not PYARROW_AIO_AVAILABLE:
              return
          tags = {"component": self.worker_id, "layer": layer_idx}
          start_time = time.perf_counter()
          log.debug(f"Archiver {self.worker_id}: Writing {len(packets)} packets for layer {layer_idx}")
          try:
               ...
               flattened_tensors = [t.flatten() for t in data_list]
               ...
               table = pa.table({
                    ...
                    # 'activation': pa.array(flattened_tensors), # Needs careful type handling
                     'activation_blob': pa.array([t.tobytes() for t in data_list], type=pa.binary()),
               ...
               await loop.run_in_executor(
                    ...
                    lambda: pq.write_table(table, filepath, compression=self.archiver_config.compression)
               )
               # async with aiofiles.open(filepath, mode='wb') as f:
               #      # Need async parquet writer, which doesn't exist easily. 

               write_latency = (time.perf_counter() - start_time) * 1000
               self.monitor.log_metric("archiver_write_latency_ms", write_latency, tags=tags)
               ...
               log.info(f"Archiver {self.worker_id}: Wrote {filepath} ({total_bytes/1e6:.2f} MB)")
               
          except Exception as e:
                log.exception(f"Error writing parquet file for layer {layer_idx}:")
                self.monitor.log_metric("archiver_error_count", 1, tags={**tags, "type": "write"})
          ...
          while not self.stop_event.is_set():
               try:
                   ...
                   if not batch:
                        ...
                        if now - self.last_flush_time > self.archiver_config.flush_interval_sec:
                             await self._flush_buffers()
                        continue

                   self._packet_count += len(batch)
                   self.monitor.log_metric("archiver_pull_count", len(batch), tags={"component": self.worker_id})
                   ...
                   if time_to_flush or size_to_flush:
                         log.debug(f"Archiver {self.worker_id} flushing buffers (Time: {time_to_flush}, Size: {size_to_flush}, Count: {buffer_size}).")
                         await self._flush_buffers()
                    ...
                    self.monitor.log_metric("archiver_error_count", 1, tags={"component": self.worker_id, "type": "loop"})
     ...
     def run(self):
         # Entry point for the process
          if not self.archiver_config.enabled or not PYARROW_AIO_AVAILABLE:
               log.warning(f"Archiver {self.worker_id} disabled by config or missing deps. Exiting.")
               return
          try:
     ...
     def shutdown(self):
         ...
         log.info(f"ArchiverWorker {self.worker_id} shutdown. Total packets: {self._packet_count}, Bytes: {self._bytes_written/1e6:.2f} MB")
...
class ArchiverManager(RestartableManager):
      ...
      def _create_and_register_worker(self, worker_id: ComponentID) -> multiprocessing.Process:
          process = multiprocessing.Process(
               target=archiver_worker_process_target,
               args=(
                    worker_id, self.config, self.data_bus, self.monitor, 
                    self.stop_event, self.all_layer_indices
               ),
               daemon=True,
               name=worker_id
          )
      ...
      def restart_worker(self, worker_id: ComponentID) -> bool:
          ...
          log.warning(f"Restarting archiver worker {worker_id} (old PID: {old_process.pid})...")
          ...
          new_process.start()
          log.info(f"Restarted archiver worker {worker_id} with new PID: {new_process.pid}")
          return True
