...
def trainer_worker_process_target(
    ...
    try:
        ...
        ):  # Only try to init CUDA if not already set to CPU
            torch.cuda.init()
    except RuntimeError as e:
        if "Found no NVIDIA driver" in str(e) or "CUDA unavailable" in str(e).lower():
            log.warning(
                f"SAETrainerWorker {worker_id}: CUDA init failed ({e}). Defaulting to CPU."
            )
    ...
    worker = SAETrainerWorker(
        worker_id, config, data_bus, monitor, stop_event, assigned_layer_indices
    )
    try:
        worker.run()
        ...
        monitor.log_metric(
            "worker_crash_count", 1, tags={"component": worker_id, "type": "trainer"}
        )
    finally:
        worker.shutdown()
...
class SAETrainerWorker:
    ...
    def _setup(self):
        log.info(
            f"SAETrainerWorker {self.worker_id} setting up for layers: {self.layer_indices}"
        )
        ...
        for layer_idx in self.layer_indices:
            log.debug(
                f"{self.worker_id}: Initializing SAE and Optimizer for layer {layer_idx}"
            )
    ...
    def _prepare_batch(
        self, packets: List[ActivationPacket]
        ...
        for layer_idx, tensor_list in grouped_tensors.items():
            if layer_idx in self.models:  # Only process layers this worker manages
                try:
                    ...
                    batch_tensor = torch.cat(tensor_list, dim=0).to(
                        self.device, non_blocking=True
                    )
                    final_batches[layer_idx] = batch_tensor
                except Exception as e:
                    log.error(
                        f"{self.worker_id}: Error stacking tensors for layer {layer_idx}: {e}"
                    )
                    self.monitor.log_metric(
                        "trainer_batch_error_count",
                        1,
    ...
    def _save_checkpoint(self, layer_idx: LayerIndex, step: GlobalStep):
        path = os.path.join(
            self.checkpoint_dir, f"sae_layer_{layer_idx}_step_{step}.pt"
        )
        try:
            self.models[layer_idx].save_weights(path)
            log.info(
                f"{self.worker_id}: Saved checkpoint for layer {layer_idx} step {step} to {path}"
            )
            self.monitor.log_metric(
                "trainer_checkpoint_count", 1, tags={**self.tags, "layer": layer_idx}
            )
        except Exception as e:
            log.error(
                f"{self.worker_id}: Failed to save checkpoint for layer {layer_idx}: {e}"
            )
            self.monitor.log_metric(
                "trainer_error_count",
                1,
                tags={**self.tags, "layer": layer_idx, "type": "checkpoint"},
            )
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self.monitor.log_metric(
                    "trainer_pull_count", len(batch_packets), tags=self.tags
                )
                ...
                for layer_idx, tensor_batch in batches.items():
                    try:
                        ...
                        log.exception(
                            f"{self.worker_id}: Error during training step for layer {layer_idx}:"
                        )
                        self.monitor.log_metric(
                            "trainer_error_count",
                            1,
                ...
                self.monitor.log_metric(
                    "trainer_error_count", 1, tags={**self.tags, "type": "loop"}
                )
...
class SAETrainerManager(RestartableManager):
    ...
    def __init__(
        ...
        log.info(
            f"SAETrainerManager initialized for {self.trainer_config.num_workers} workers, layers: {self.all_layer_indices}"
        )
    ...
    def _create_and_register_worker(
        self, worker_id: ComponentID, layers: List[LayerIndex]
    ...
    def initialize(self):
        ...
        for i in range(self.trainer_config.num_workers):
            ...
            if layers:
                log.info(
                    f"Initializing SAETrainerWorker: {worker_id} for layers {layers}"
                )
    ...
    def restart_worker(self, worker_id: ComponentID) -> bool:
        ...
        log.warning(
            f"Restarting trainer worker {worker_id} (old PID: {old_process.pid}) for layers {assigned_layers}..."
        )
        ...
        new_process.start()
        log.info(
            f"Restarted trainer worker {worker_id} with new PID: {new_process.pid}"
        )
        return True
