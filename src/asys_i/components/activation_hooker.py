...
class ActivationHooker:
    ...
    def __init__(
        ...
        if self.profile == RunProfile.HPC and CUDA_AVAILABLE:
            ...
            log.info(
                "HPC mode: Initialized low-priority CUDA stream for hook processing."
            )

        log.info(
            f"ActivationHooker initialized. Profile: {self.profile}, Sampling: {self._current_sampling_rate:.2f}, Layers: {self.hook_config.layers_to_hook}"
        )
    ...
    def _handle_backpressure(self, success: bool):
        ...
        if not success:
            ...
            if (
                now - self._last_backpressure_time
                > self.hook_config.backpressure_debounce_sec
            ):
                old_rate = self._current_sampling_rate
                # Exponential backoff
                self._current_sampling_rate = max(
                    self.hook_config.min_sampling_rate, old_rate * 0.7
                )
                log.warning(
                    f"BACKPRESSURE: DataBus full. Reducing sampling rate: {old_rate:.3f} -> {self._current_sampling_rate:.3f}"
                )
                self.monitor.log_metric("hook_backpressure_event_count", 1, tags=tags)
                self.monitor.log_metric(
                    "hook_sampling_rate", self._current_sampling_rate, tags=tags
                )
    ...
    def _find_module_by_index(self, layer_idx: LayerIndex) -> Optional[nn.Module]:
        ...
        try:
            ...
            if hasattr(self.model, "layers"):
                return self.model.layers[layer_idx]  # type: ignore
            if hasattr(self.model, "h"):
                return self.model.h[layer_idx]  # type: ignore
            if hasattr(self.model, "blocks"):
                return self.model.blocks[layer_idx]  # type: ignore
            # MOCK model access
            ...
            log_once(
                logging.WARNING,
                f"Hooker: Cannot find standard layer list, attempting direct access for layer {layer_idx}. Define hook_point name mapping.",
            )
    ...
    def _extract_tensor(self, output: Any) -> Optional[torch.Tensor]:
        ...
        log_once(
            logging.DEBUG,
            f"Could not extract primary tensor from hook output type: {type(output)}",
        )
        return None
    ...
    def _create_hook_fn(self, layer_idx: LayerIndex):
        ...
        def hook(module: nn.Module, input: Tuple, output: Any):
            ...
            if (
                not self._attached
                or self._global_step_ref is None
                or not self.data_bus.is_ready()
            ):
            ...
            try:
                ...
                if tensor is None:
                    self.monitor.log_metric(
                        "hook_error_count", 1, tags={**tags, "reason": "extract_fail"}
                    )
                    return
                ...
                if (
                    self.profile == RunProfile.HPC
                    and self.cuda_stream
                    and tensor_detached.is_cuda
                ):
                    ...
                    with torch.cuda.stream(self.cuda_stream):
                        ...
                        if packet_data.is_cuda:
                            ...
                            packet_data = packet_data.to(
                                device="cpu", non_blocking=True
                            )

                        # Create packet *after* tensor processing
                        packet = create_activation_packet(
                            layer_idx, step, packet_data, self.profile, meta
                        )
                        ...
                        self.monitor.log_metric(
                            "hook_push_latency_ms", push_latency, tags=tags
                        )
                ...
                if success:
                    ...
                    if isinstance(packet_data, torch.Tensor):
                        ...
                        self.monitor.log_metric(
                            "hook_throughput_bytes", nbytes, tags=tags
                        )
                else:
                    self.monitor.log_metric(
                        "hook_packet_drop_count", 1, tags=tags
                    )  # Backpressure drop

            except Exception:
                log.exception(f"Error in hook function for layer {layer_idx}:")
                self.monitor.log_metric(
                    "hook_error_count", 1, tags={**tags, "reason": "exception"}
                )
    ...
    def attach(self, global_step_ref: Callable[[], GlobalStep]):
        ...
        for layer_idx in self.hook_config.layers_to_hook:
            ...
            if module:
                ...
                log.info(
                    f"Attached forward hook to layer {layer_idx} ({module.__class__.__name__})"
                )
            else:
                log.error(
                    f"Could not find module for layer index {layer_idx}. Hook not attached."
                )
                self.monitor.log_metric(
                    "hook_attach_error_count", 1, tags={"layer": layer_idx}
                )
