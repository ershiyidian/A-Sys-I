...
class BaseDataBus(ABC):
    ...
    def __init__(self,
         ...
         log.info(f"Initialized DataBus: {self.__class__.__name__} with buffer size {config.buffer_size_per_shard}, shards {config.num_shards}")
    ...
    @abstractmethod
    def register_consumer(self, consumer_id: ConsumerID, layer_indices: List[int]) -> None:
...
class NoOpDataBus(BaseDataBus):
     ...
     def pull_batch(self, consumer_id: ConsumerID, batch_size: Optional[int] = None, timeout: Optional[float] = None) -> List[ActivationPacket]:
          return []
     def register_consumer(self, consumer_id: ConsumerID, layer_indices: List[int]) -> None: pass
     def get_stats(self) -> Dict[str, Any]: return {"depth": 0, "drops": "N/A"}
     def shutdown(self) -> None:
         super().shutdown()
