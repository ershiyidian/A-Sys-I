...
)  # TODO: Review if this is still needed or should be instance member
...
class ExperimentPipeline:
    ...
    def __init__(self, config: MasterConfig):
        ...
        self.shared_heartbeats_dict: Optional[DictProxy[str, Any]] = None
    ...
    def _resolve_auto_configs(self) -> None:
        """
        ...
        if self.config.sae_model.d_in == "auto":
            assert (
                self.ppo_host
            ), "PPOHost must be initialized before auto-detecting d_in"
            assert (
                self.monitor is not None
            ), "Monitor must be initialized before resolving auto configs"
            ...
            try:
                ...
                if hasattr(host_model, "config") and hasattr(
                    host_model.config, "hidden_size"
                ):
                    val = getattr(host_model.config, "hidden_size", None)
                    if isinstance(val, int):
                        detected_dim = val
                    else:
                        raise TypeError(f"Expected int for hidden_size, got {type(val)}")
                elif hasattr(host_model, "config") and hasattr(
                    host_model.config, "d_model"
                ):
                    val = getattr(host_model.config, "d_model", None)
                    if isinstance(val, int):
                        detected_dim = val
                    else:
                        raise TypeError(f"Expected int for d_model, got {type(val)}")
                ...
                # This is a heuristic and might need refinement for complex models
                    ...
                    hasattr(host_model, "layers")
                    and isinstance(host_model.layers, torch.nn.ModuleList)
                    and len(host_model.layers) > 0
                ):
                    first_layer_module = host_model.layers[
                        0
                    ]  # Try to find a common layer block
                    # Look for a linear layer within it
                    first_linear = next(
                        (
                            m
                            for m in first_layer_module.modules()
                    ...
                    if first_linear:
                        ...
                        raise RuntimeError(
                            "Cannot automatically determine model hidden dimension (d_in). No 'hidden_size', 'd_model', or common Linear layers found."
                        )

                if detected_dim <= 0:
                    raise RuntimeError(
                        f"Detected d_in value {detected_dim} is invalid."
                    )

                log.info(
                    f"Auto-detected host model d_in: {detected_dim}. Updating SAE config."
                )
                self.config.sae_model.d_in = detected_dim
                self.monitor.log_metric("sae_model_d_in_auto_detected", detected_dim)
            except Exception as e:
                log.error(
                    f"Failed to auto-detect d_in: {e}. Please specify `sae_model.d_in` in config if 'auto' fails."
                )
                raise
        ...
        if (
            self.config.run_profile == RunProfile.HPC
            and res_config.apply_bindings
            and res_config.allocation_strategy == "auto"
            and not res_config.cpu_affinity_map
        ):  # Only auto-generate if no map provided
            ...
            if not os.name == "posix" or not hasattr(psutil, "cpu_count"):
                log.warning(
                    "Auto CPU affinity requires Linux and psutil. Skipping auto-binding."
                )
                self.monitor.log_metric(
                    "resource_binding_auto_skip",
                    1,
                    tags={"reason": "not_linux_or_psutil"},
                )
                return
            ...
            if num_logical_cores is None or num_logical_cores == 0:
                log.error(
                    "Could not determine number of logical CPU cores. Cannot auto-assign."
                )
                self.monitor.log_metric(
                    "resource_binding_auto_error", 1, tags={"reason": "no_cpu_count"}
                )
                return

            num_trainers = self.config.sae_trainer.num_workers
            num_archivers = (
                1 if self.config.archiver.enabled else 0
            )  # Assume one archiver worker
            ...
            if host_actual_cores > 0:
                ...
                log.debug(
                    f"Assigned {host_actual_cores} cores to host_process: {core_map['host_process']}"
                )
            ...
            if num_archivers > 0 and remaining_cores:
                ...
                if archiver_actual_cores > 0:
                    core_map["archiver_worker_0"] = remaining_cores[
                        :archiver_actual_cores
                    ]
                    remaining_cores = remaining_cores[archiver_actual_cores:]
                    log.debug(
                        f"Assigned {archiver_actual_cores} cores to archiver_worker_0: {core_map['archiver_worker_0']}"
                    )
            ...
            if remaining_cores:
                ...
                if cores_per_trainer == 0:
                    log.warning(
                        f"Not enough remaining cores ({len(remaining_cores)}) to assign at least 1 core per trainer. Trainers may share cores."
                    )
                    cores_per_trainer = 1
                ...
                for i in range(num_trainers):
                    ...
                    assigned_cores = remaining_cores[
                        start_idx : min(
                            start_idx + cores_per_trainer, len(remaining_cores)
                        )
                    ]
                    if assigned_cores:
                        core_map[worker_id] = assigned_cores
                        log.debug(
                            f"Assigned {len(assigned_cores)} cores to {worker_id}: {assigned_cores}"
                        )
                    else:
                        log.warning(
                            f"No cores could be assigned to {worker_id}. It will run on unpinned cores."
                        )
            else:
                log.warning(
                    "No remaining cores for SAE trainer workers. They will run on unpinned cores."
                )

            self.config.resource_manager.cpu_affinity_map = core_map
            self.monitor.log_metric(
                "resource_binding_auto_success",
                1,
                tags={"allocated_cores": len(core_map)},
            )
            log.info(f"Generated CPU affinity map: {core_map}")

        else:
            log.info(
                "CPU affinity map either not requested for auto-generation or manually specified."
            )
    ...
    def setup(self) -> None:
        ...
        try:
            ...
            log.info(
                "Initializing Multiprocessing Manager for heartbeats in ExperimentPipeline setup..."
            )
            self.heartbeat_manager = multiprocessing.Manager()
            assert (
                self.heartbeat_manager is not None
            ), "Heartbeat manager failed to initialize"
            self.shared_heartbeats_dict = self.heartbeat_manager.dict()
            assert (
                self.shared_heartbeats_dict is not None
            ), "Shared heartbeats dict failed to initialize"

            # ORDER IS NOW CRITICAL FOR AUTO-CONFIG RESOLUTION
            # 1. Monitor (dependency for all others - now receives the shared dict)
            assert self.shared_heartbeats_dict is not None # MyPy needs this for the cast
            self.monitor = create_monitor(self.config, dict(self.shared_heartbeats_dict))
            self.monitor.log_hyperparams(self.config.dict()) # type: ignore[no-untyped-call]
            ...
            self.ppo_host = PPOHostProcess(self.config, self.monitor, self.stop_event) # type: ignore[arg-type]

            # 4. *** INTELLIGENCE UPGRADE STEP: RESOLVE "AUTO" CONFIGS ***
            self._resolve_auto_configs() # type: ignore[no-untyped-call]

            # 5. Hooker (now uses potentially updated d_in from config)
            self.hooker = ActivationHooker(
                self.ppo_host.get_model(), self.config, self.data_bus, self.monitor # type: ignore[no-untyped-call]
            )

            all_layers = self.config.hook.layers_to_hook
            if not all_layers:
                log.warning(
                    "No layers configured to hook. SAE training and archiving will be idle."
                )
            ...
            self.trainer_manager.initialize() # type: ignore[no-untyped-call]

            # 7. Archiver Manager (now uses fully resolved config, including cpu_affinity)
            self.archiver_manager = ArchiverManager(
                self.config, self.data_bus, self.monitor, self.stop_event, all_layers
            )
            self.archiver_manager.initialize() # type: ignore[no-untyped-call]

            # 8. Watchdog (needs monitor and managers)
            managed_components_for_watchdog: Dict[str, Any] = {}
            if self.trainer_manager:
                managed_components_for_watchdog["trainer"] = self.trainer_manager
            if self.archiver_manager:
                managed_components_for_watchdog["archiver"] = self.archiver_manager
            self.watchdog = Watchdog(
                self.monitor,
                self.config.monitor,
                managed_components=managed_components_for_watchdog,
            )
            self.monitor.register_component(self.ppo_host.component_id) # type: ignore[no-untyped-call]

            self._is_setup = True
            duration = time.time() - start_time
            log.info(f">>> PIPELINE SETUP COMPLETE ({duration:.2f}s) <<<")
            self.monitor.log_metric("pipeline_setup_duration_sec", duration) # type: ignore[no-untyped-call]

        except Exception:
            log.exception("Error during pipeline setup:")
            if self.monitor:
                self.monitor.log_metric( # type: ignore[no-untyped-call]
                    "pipeline_error_count", 1, tags={"stage": "setup"}
                )
            self.shutdown() # type: ignore[no-untyped-call]
            raise
    ...
    def run(self) -> None:
        ...
        if self.stop_event.is_set():
            log.warning(
                "Run called but stop_event is already set, perhaps from previous shutdown."
            )
            return
        ...
        try:
            assert (
                self.watchdog
                and self.trainer_manager
                and self.archiver_manager
                and self.ppo_host
                and self.hooker
            )

            self.watchdog.start() # type: ignore[no-untyped-call]
            self.trainer_manager.start_all() # type: ignore[no-untyped-call]
            self.archiver_manager.start_all() # type: ignore[no-untyped-call]

            self.ppo_host.run_training_loop(self.hooker) # type: ignore[no-untyped-call]
            ...
            if self.monitor:
                self.monitor.log_metric("pipeline_run_duration_sec", duration) # type: ignore[no-untyped-call]

        except Exception:
            log.exception("Error during pipeline run:")
            if self.monitor:
                self.monitor.log_metric( # type: ignore[no-untyped-call]
                    "pipeline_error_count", 1, tags={"stage": "run"}
                )
            self.stop_event.set()
            raise
    ...
    def shutdown(self) -> None:
        ...
        try:
            if self.ppo_host:
                self.ppo_host.shutdown() # type: ignore[no-untyped-call]
            if self.hooker:
                self.hooker.shutdown() # type: ignore[no-untyped-call]
            if self.archiver_manager:
                self.archiver_manager.stop_all() # type: ignore[no-untyped-call]
            if self.trainer_manager:
                self.trainer_manager.stop_all() # type: ignore[no-untyped-call]
            if self.watchdog and self.watchdog.is_alive():
                self.watchdog.stop() # type: ignore[no-untyped-call]
            time.sleep(2)
            if self.data_bus:
                self.data_bus.shutdown() # type: ignore[no-untyped-call]
            if self.monitor:
                self.monitor.shutdown() # type: ignore[no-untyped-call]
            ...
            if hasattr(self, "heartbeat_manager") and self.heartbeat_manager:
                ...
                try:
                    if hasattr(self.heartbeat_manager, "_process") and self.heartbeat_manager._process.is_alive():
                        self.heartbeat_manager.shutdown() # type: ignore[no-untyped-call]
                    elif not hasattr(
                        self.heartbeat_manager, "_process"
                    ):  # For some manager types or if already shutdown
