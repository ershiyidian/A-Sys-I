...
# Import core types
from asys_i.common.types import RunProfile, MonitorType, DataBusType, LayerIndex, WorkerID, ComponentID
...
class SAEModelConfig(BaseModel):
    ...
    d_in: Union[int, str] = Field("auto", description="Input dimension. 'auto' detects from host model.")
    d_sae: int = Field(768 * 4, description="SAE feature dimension (expansion factor * d_in)")
    ...
    @validator('d_in')
    def d_in_must_be_positive_or_auto(cls, v):
...
class SAETrainerConfig(BaseModel):
    ...
    num_workers: int = Field(1, description="Number of parallel SAETrainerWorker processes")
    layers_per_worker: Union[str, int] = Field("auto", description="Distribute layers among workers")
...
class HookConfig(BaseModel):
     ...
     sampling_rate: float = Field(1.0, ge=0.0, le=1.0, description="Probability of sampling an activation")
     # HPC only
...
class DataBusConfig(BaseModel):
     type: DataBusType
     # Common
     buffer_size_per_shard: int = Field(2**18, description="Max packets in queue/buffer per shard")
     pull_batch_size_max: int = 4096 # Max packets a consumer can pull at once
     push_timeout_sec: float = 0.001 # Small timeout for push to enable fast backpressure
     ...
     shared_memory_size_gb: float = Field(8.0, description="Size of shared memory for tensors (HPC)")

     @validator("type", pre=True, always=True)
     def validate_type(cls, v, values, config, field):
     ...
     def check_hpc_params(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if db_type == DataBusType.PYTHON_QUEUE and values.get("num_shards", 1) > 1:
             log.warning("PythonQueueBus does not support sharding. num_shards will be ignored.")
             values["num_shards"] = 1
        elif db_type == DataBusType.CPP_SHARDED_SPMC and values.get("num_shards", 1) < 1:
...
class ResourceManagerConfig(BaseModel):
    ...
    allocation_strategy: str = Field("auto", description="'auto' for heuristic-based core assignment, 'manual' to use map below.")
    ...
    def check_allocation_strategy(cls, v):
...
class MasterConfig(BaseModel):
    ...
    def set_defaults_based_on_profile(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'data_bus' not in values or 'type' not in values['data_bus']:
             ...
             log.info(f"Defaulting DataBus type to {default_db_type} for profile {profile}")
        ...
        if 'monitor' not in values or 'type' not in values['monitor']:
            ...
            log.info(f"Defaulting Monitor type to {default_mon_type} for profile {profile}")
    ...
    def cross_validate_profile(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        resource_config: Optional[ResourceManagerConfig] = values.get("resource_manager")
        ...
        if profile == RunProfile.HPC:
            if data_bus_config and data_bus_config.type != DataBusType.CPP_SHARDED_SPMC:
                 log.warning(f"HPC profile active, but DataBus type is {data_bus_config.type}. Performance SLA may not be met.")
            if monitor_config and monitor_config.type != MonitorType.PROMETHEUS:
                 log.warning(f"HPC profile active, but Monitor type is {monitor_config.type}. High-resolution monitoring unavailable.")
            if resource_config and resource_config.apply_bindings and resource_config.allocation_strategy == "manual" and not resource_config.cpu_affinity_map:
                 log.warning("HPC profile: resource_manager.apply_bindings is True and allocation_strategy is 'manual', but cpu_affinity_map is empty. Binding will not occur as expected.")
            if hook_config and hook_config.gpu_kernel_mode != "NONE":
                  log.info(f"HPC profile: GPU kernel preprocessing mode enabled: {hook_config.gpu_kernel_mode}")
             ...
             if monitor_config and monitor_config.type == MonitorType.PROMETHEUS:
                   log.warning("CONSUMER profile: PrometheusMonitor selected, ensure prometheus-client is installed.")
             if resource_config and resource_config.apply_bindings and resource_config.allocation_strategy == "manual":
                   log.warning("CONSUMER profile: resource_manager.apply_bindings is True, but CPU/NUMA binding is primarily for HPC and might not work or be effective.")
             if hook_config and hook_config.gpu_kernel_mode != "NONE":
                    raise ValueError(f"CONSUMER profile cannot use GPU kernel mode {hook_config.gpu_kernel_mode}. Set to NONE.")
             if sae_model_config and sae_model_config.d_in == "auto":
                 ...
                 log.warning("CONSUMER profile with `sae_model.d_in: auto`. Ensure the host model is loaded for detection.")
...
def load_config(
    config_path: str,
    base_config_path: Optional[str] = "configs/base.yaml"
 ) -> MasterConfig:
    """
    ...
    try:
        ...
        log.info(f"Configuration loaded and validated successfully. Run Profile: {config.run_profile}")
