...
class BenchmarkSuite:
    def __init__(self, config: MasterConfig):
        ...
        if not config.hook.layers_to_hook:
            log.warning(
                "No layers defined in hook.layers_to_hook. Benchmarks may not capture meaningful data."
            )
            # Default to first few layers for sanity if not set
            config.hook.layers_to_hook = [0, 1, 2]

        self.config = config
        self.pipeline: Optional[ExperimentPipeline] = (
    ...
    def test_hook_latency(self):
        ...
        assert (
            self.pipeline
            and self.pipeline.trainer_manager
            and self.pipeline.archiver_manager
            and self.pipeline.monitor
        )
        ...
        def temp_metric_collector(name, value, step=None, tags=None):
            ...
            original_log_metric(
                name, value, step, tags
            )  # Still log to original monitor

        self.pipeline.monitor.log_metric = temp_metric_collector

        # Run host for a short duration to generate enough samples
        self._run_host_for_duration(
            duration_sec=10
        )  # 10 seconds should generate many packets
        ...
        if not latency_metrics:
            ...
            self.results["hook_latency"] = {
                "error": "No data collected. Check hook config and host model activity."
            }
            return
        ...
        log.info(
            f"Hook Latency Results (n={len(latency_metrics)}): P99={p99:.3f}ms, P50={p50:.3f}ms, Avg={avg:.3f}ms"
        )
    ...
    def test_bus_throughput(self):
        ...
        def dummy_consumer():
            ...
            all_layers = (
                self.config.hook.layers_to_hook
                if self.config.hook.layers_to_hook
                else list(range(10))
            )  # Heuristic if empty
            self.pipeline.data_bus.register_consumer("benchmark_consumer", all_layers)

            while not consumer_stop_event.is_set():
                packets = self.pipeline.data_bus.pull_batch(
                    "benchmark_consumer", timeout=0.5
                )
                for p in packets:
                    if isinstance(p["data"], torch.Tensor):
                        ...
                        log.debug(
                            "Dummy consumer received non-tensor data type, skipping byte count."
                        )
        ...
        self.results["bus_throughput"] = {
            "gbps": throughput_gbps,
            "total_gb_transferred": pulled_bytes / 1e9,
        }
        log.info(
            f"DataBus Throughput Results: {throughput_gbps:.4f} GB/s (Total: {pulled_bytes / 1e9:.2f} GB)"
        )
    ...
    def test_isolation(self):
        ...
        baseline_steps_completed = self._run_host_for_duration(
        ...
        loaded_steps_completed = self._run_host_for_duration(
            duration_sec=test_duration_sec
        )
        ...
        if baseline_steps_per_sec > 0:
            performance_impact_percent = (
                (baseline_steps_per_sec - loaded_steps_per_sec) / baseline_steps_per_sec
            ) * 100
        ...
        log.info(
            f"Host Isolation Results: Performance impact = {performance_impact_percent:.2f}%"
        )
    ...
    def generate_report(self):
        ...
        print("=" * 50)
        print(
            f"Profile: {self.config.run_profile.value}, DataBus: {self.config.data_bus.type.value}"
        )
        print(f"Host Model d_in: {self.config.sae_model.d_in}")
        print(f"Trainer Workers: {self.config.sae_trainer.num_workers}")
        print(
            f"CPU Affinity: {self.config.resource_manager.allocation_strategy} ({self.config.resource_manager.cpu_affinity_map if self.config.resource_manager.apply_bindings else 'Disabled'})"
        )
        ...
        print(
            f"  - Baseline Performance: {isolation.get('baseline_steps_per_sec',-1):.2f} steps/sec"
        )
        print(
            f"  - Performance w/ A-Sys-I: {isolation.get('loaded_steps_per_sec',-1):.2f} steps/sec"
        )
        print(
            f"  - Performance Impact: {isolation.get('performance_impact_percent',-1):.2f} %"
        )
        print("=" * 50)
