...
class Watchdog(threading.Thread):
    ...
    def __init__(
        ...
        log.info(
            f"Watchdog initialized. Check interval: {self.check_interval}s, Timeout: {self.timeout}s"
        )
    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                components_to_check = (
                    self._monitored_ids
                )  # Only check managed components

                for component_id in components_to_check:
                    last_seen = heartbeats.get(component_id)
                    if last_seen is None:
                        log.warning(
                            f"Component {component_id} is monitored but never sent a heartbeat!"
                        )
                    ...
                    if time_since_last_seen > self.timeout:
                        ...
                        log.debug(
                            f"Component {component_id} OK (last seen {time_since_last_seen:.1f}s ago)"
                        )
        ...
        for manager_name, manager in self.managed_components.items():
            try:
                if component_id in manager.get_worker_ids():
                    log.warning(
                        f"Requesting manager '{manager_name}' to restart '{component_id}'"
                    )
                    ...
                    if success:
                        log.info(
                            f"Manager '{manager_name}' reported successful restart for '{component_id}'."
                        )
                        restarted = True
                        ...
                        log.error(
                            f"Manager '{manager_name}' failed to restart '{component_id}'."
                        )
            except Exception:
                log.exception(
                    f"Error during restart attempt for {component_id} via manager {manager_name}:"
                )

        if not restarted:
            log.error(
                f"Could not find a manager to restart component {component_id} or restart failed."
            )
            self.monitor.log_metric(
                "watchdog_restart_failed", 1, tags={"component": component_id}
            )
