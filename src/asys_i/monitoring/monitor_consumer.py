...
try:
    ...
    TENSORBOARD_AVAILABLE = True
     ...
     try:
         ...
         logging.warning("TensorBoard/TensorBoardX not found. TensorBoard logging disabled.")
...
class LoggingCSVTensorBoardMonitor(BaseMonitor):
    ...
    def _write_csv(self, timestamp:float, name:str, value:float, step:Optional[int], tags:Optional[Dict[str,Any]]):
         ...
         with self._lock:
              ...
              # Auto-flush check
              if time.time() - self._last_flush > self.monitor_config.metrics_flush_interval_sec:
         ...
         if self._writer:
              try:
                 with self._lock:
                     ...
                     self._writer.add_scalars(main_tag, scalars_dict, global_step=step or 0)
              except Exception as e:
    ...
    def log_hyperparams(self, params: Dict[str, Any], metrics: Optional[Dict[str, float]] = None) -> None:
         ...
         if self._writer:
              try:
                   ...
                   valid_params = {k: v for k,v in params.items() if isinstance(v, (int, float, str, bool, torch.Tensor))}
                   valid_metrics = metrics or {"dummy_metric": 0.0}
                   valid_metrics = {k: v for k,v in valid_metrics.items() if isinstance(v, (int, float, str, bool, torch.Tensor))}
                   with self._lock:
    ...
    def heartbeat(self, component_id: ComponentID) -> None:
         ...
         # Also log as metric
         self.log_metric("component_heartbeat", time.time(), tags={"component": component_id})
