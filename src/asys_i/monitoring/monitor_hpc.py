...
from asys_i.hpc import check_hpc_prerequisites, PROMETHEUS_AVAILABLE as HPC_PROM_AVAIL
from asys_i.monitoring.monitor_interface import BaseMonitor
from asys_i.orchestration.config_loader import MonitorConfig, ProjectConfig, MasterConfig
...
LATENCY_BUCKETS_MS = (0.1, 0.5, 1, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000)
# Standard BUCKETS
...
class PrometheusMonitor(BaseMonitor):
     ...
     def __init__(self,
         ...
         try:
            ...
            # TODO: Handle server start in multi-process scenario better (e.g. only main process)
             ...
             log.info(f"Prometheus metrics server started at http://0.0.0.0:{self.port}/metrics")
         except OSError as e:
              log.warning(f"Could not start Prometheus server on port {self.port} (maybe already running?): {e}")
              self._server_started = False
     ...
     def _infer_metric_type(self, name:str) -> Any:
         ...
         for suffix, metric_type in METRIC_TYPE_MAP.items():
              if name_lower.endswith(f"_{suffix}"):
                   return metric_type
         # Special cases
         if "latency" in name_lower: return Histogram
         if "count" in name_lower: return Counter
     ...
     def _get_or_create_metric(self,
          ...
          with self._lock:
               ...
               log.debug(f"Creating new Prometheus metric: {metric_name} (Type: {inferred_type.__name__}) Labels: {label_names}")
               ...
               try:
                    ...
                    # Handle cases where metric exists with different labels or from other process
                     log.warning(f"Could not create or retrieve metric {metric_name}: {e}. Checking REGISTRY.")
                     if metric_name in REGISTRY._names_to_collectors:
                          collector = REGISTRY._names_to_collectors[metric_name]
                          self._metrics_cache[metric_name] = collector # cache existing
                          return collector
                     else:
                          log.error(f"Failed to access metric {metric_name} even from REGISTRY.")
                          raise
     ...
     def log_metric(self,
         ...
         try:
             ...
             if isinstance(metric, Counter):
               ...
               log.error(f"Error logging Prometheus metric {name} with labels {label_values}: {e}")
     ...
     def log_hyperparams(self, params: Dict[str, Any], metrics: Optional[Dict[str, float]] = None) -> None:
          # Prometheus uses 'info' metrics for static labels
           info_metric = self._get_or_create_metric("experiment_info", "Experiment Hyperparameters", list(params.keys()), Gauge)
