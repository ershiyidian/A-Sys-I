...
def signal_handler(sig, frame):
    ...
    log.warning(
        f"!!! Signal {signal.Signals(sig).name} received. Initiating graceful shutdown... !!!"
    )
...
def main(
    ...
    try:
        ...
        print(f"CRITICAL ERROR: Config file not found: {fnf_error}", file=sys.stderr)
        exit_code = 1
    except Exception:
...
if __name__ == "__main__":
    ...
    multiprocessing.set_start_method(
        "spawn", force=True
    )  # 'spawn' is safer with CUDA/torch
    app()
